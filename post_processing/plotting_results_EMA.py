...
def plot_results_bridge(
    df_results: pd.DataFrame, entity_number: int, years_kde: list[int], save_fig: bool = False
) -> None:
    # Check input types
    ...
    for idx, year in enumerate(YEARS_KDE):
        ...
        if year == df_results.index.max():
            ax1.text(year, max_value, str(year), color=colors2[idx], fontsize=8, ha='center', va='bottom')
        else:
            ax1.text(year, max_value + 0.1, str(year), color=colors2[idx], fontsize=8, ha='left', va='bottom')
...
def plot_results_road_network(df_results: pd.DataFrame, years_kde: list[int], save_fig: bool = False) -> None:
    ...
    if save_fig:
        fig_format = "svg"
        plt.savefig(PLOT_DIR / f"{TRAFFIC_TYPE}_VKT.{fig_format}", dpi=600, bbox_inches='tight')
...
def get_spread_stats(df_results: pd.DataFrame, years_kde: list[int]) -> pd.DataFrame:
    ...
    spread_stats = pd.DataFrame(columns=["Year", "Mean", "Std", "Min", "25%", "50%", "75%", "Max", "IQR", "MAD"])
