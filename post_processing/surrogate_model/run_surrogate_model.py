...
def predict_from_df(pipeline: Pipeline, df: pd.DataFrame, batch_size: int = 500) -> np.ndarray:
    """
...
def plot_prediction_distribution(predictions: np.ndarray) -> None:
        ...
        n, bins, patches = ax.hist(predictions, bins=50, color='#2E86AB', 
                                     alpha=0.8, edgecolor='black', linewidth=0.3)  # Reduced from 0.5
        
        ax.set_xlabel('Predicted Value')
        ax.set_ylabel('Frequency')
        ax.set_title('Distribution of Surrogate Model Predictions', pad=8)  # Reduced padding
        ...
        ax.axvline(mean_val, color='red', linestyle='--', linewidth=1.2,  # Reduced from 1.5
                   label=f'Mean = {mean_val:.2e}')  # Changed to scientific notation
