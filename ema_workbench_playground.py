...
from ema_workbench import Model, RealParameter, IntegerParameter, CategoricalParameter, Constant, ScalarOutcome, ema_logging, MultiprocessingEvaluator, perform_experiments 
import os
...
def some_model(scenario_file=None, dummy=None):
    ...
    return {f"{scenario_file}": scenario_file_exists, f"{output_dir}": output_dir_exists}
...
def main():
    ...
    scenario_paths = {f"Scenario_{i}": f"data/scenarios_ema/ema_road_model_19_03_2024_scenario_{i}.json" for i in range(NUMBER_OF_EXPERIMENTS)}
    output_paths = {key: path.replace(".json", "") for key, path in scenario_paths.items()}

    # specify uncertainties
    model.uncertainties = [IntegerParameter("dummy", 0, 10000000)]

    # specify levers
    model.levers = [CategoricalParameter("scenario_file", list(scenario_paths.values()))]
