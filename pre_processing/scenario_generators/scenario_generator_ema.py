...
DISPLAY_NAME = f"EMA road model - Scenario {SCENARIO_NUMBER} - {NOW_STRING_DATE.replace('_', '/')}"
...
def generate(name: str, display_name: str, scenario_parameters: str, name_short: str):
    config = make_config(
        name=name,
        display_name=display_name,
    )
...
def generate_and_output_multiple_scenarios(num_scenarios: int, path=None):
    ...
    for i in range(num_scenarios):
        scenario_number = i
        name = f"ema_road_model_{NOW_STRING_DATE}_scenario_{i:0{length_num_scenarios}d}"
        display_name = f"EMA road model - Scenario {scenario_number} - {NOW_STRING_DATE}"
        name_short = f"scenario_{i:0{length_num_scenarios}d}"
        scenario_global = f"{name_short}_global_parameters"

        result = json.dumps(generate(name, display_name, scenario_global, name_short), indent=2)
...
if __name__ == "__main__":
    ...
    path = pathlib.Path(sys.argv[2]) if len(sys.argv) > 2 else pathlib.Path("/home/moorens/code/data/scenarios_ema")
